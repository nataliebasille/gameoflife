...
var $author$project$World$Dead = {$: 'Dead'};
...
var $author$project$World$create = F2(
	function (rows, columns) {
		return {
			board: A2($elm$core$List$repeat, rows * columns, $author$project$World$Dead),
			cellSize: 40,
			columns: columns,
			rows: rows
		};
	});
var $author$project$Messages$SetContentViewport = function (a) {
	return {$: 'SetContentViewport', a: a};
};
...
var $author$project$Model$getContentViewport = A2(
	$elm$core$Task$attempt,
	$author$project$Messages$SetContentViewport,
	A2(
		$elm$core$Task$andThen,
		function (viewport) {
			return $elm$core$Task$succeed(
				_Utils_Tuple2(viewport.viewport.width, viewport.viewport.height));
		},
		$elm$browser$Browser$Dom$getViewportOf('page-content')));
var $author$project$Model$init = function (_v0) {
	return _Utils_Tuple2(
		{
			contentViewport: _Utils_Tuple2(0, 0),
			playing: false,
			speedMultiplier: 1,
			world: A2($author$project$World$create, 20, 20)
		},
		$author$project$Model$getContentViewport);
};
var $author$project$Messages$NextGeneration = {$: 'NextGeneration'};
...
var $author$project$Subscriptions$subscriptions = function (model) {
	return model.playing ? A2(
		$elm$time$Time$every,
		1000 / model.speedMultiplier,
		function (_v0) {
			return $author$project$Messages$NextGeneration;
		}) : $elm$core$Platform$Sub$none;
};
var $author$project$World$Alive = {$: 'Alive'};
var $author$project$World$calculcateNextCellState = function (_v0) {
	var aliveNeighborCount = _v0.a;
	var state = _v0.b;
	if (state.$ === 'Dead') {
		return (aliveNeighborCount === 3) ? $author$project$World$Alive : $author$project$World$Dead;
	} else {
		return (aliveNeighborCount < 2) ? $author$project$World$Dead : ((aliveNeighborCount <= 3) ? $author$project$World$Alive : $author$project$World$Dead);
	}
};
var $author$project$World$coordinatesToIndex = F2(
	function (rows, _v0) {
		var row = _v0.a;
		var column = _v0.b;
...
var $author$project$World$indexToCoordinates = F3(
	function (rows, columns, index) {
		return _Utils_Tuple2(
			(index / columns) | 0,
			A2($elm$core$Basics$modBy, rows, index));
	});
...
var $author$project$World$calculateNextGeneration = F3(
	function (rows, columns, world) {
		var updateAliveNeighborsHash = F2(
			function (index, hash) {
				var _v2 = A3($author$project$World$indexToCoordinates, rows, columns, index);
				var row = _v2.a;
				var column = _v2.b;
				var viableNeighbors = A2(
					$elm$core$List$map,
					$author$project$World$coordinatesToIndex(rows),
					A2(
						...
						_List_fromArray(
							[
								_Utils_Tuple2(row - 1, column - 1),
								_Utils_Tuple2(row - 1, column),
								_Utils_Tuple2(row - 1, column + 1),
								_Utils_Tuple2(row, column - 1),
								_Utils_Tuple2(row, column + 1),
								_Utils_Tuple2(row + 1, column - 1),
								_Utils_Tuple2(row + 1, column),
								_Utils_Tuple2(row + 1, column + 1)
				...
				return A3(
					...
					F2(
						function (neighbor, array) {
							var _v3 = A2($elm$core$Array$get, neighbor, array);
							if (_v3.$ === 'Just') {
								var _v4 = _v3.a;
								var value = _v4.a;
								var state = _v4.b;
		...
		var calculateNumberOfAliveNeighborsForEveryCell = function (listOfStates) {
			return $elm$core$Array$toList(
				A3(
					$elm$core$List$foldl,
					updateAliveNeighborsHash,
					$elm$core$Array$fromList(
						A2(
							$elm$core$List$map,
							function (state) {
								return _Utils_Tuple2(0, state);
							},
							listOfStates)),
					A2(
						$elm$core$List$map,
						...
						A2(
							...
							function (_v0) {
								var state = _v0.b;
								return _Utils_eq(state, $author$project$World$Alive);
							},
							A2($elm$core$List$indexedMap, $elm$core$Tuple$pair, listOfStates)))));
		};
		return A2(
			$elm$core$List$map,
			$author$project$World$calculcateNextCellState,
			calculateNumberOfAliveNeighborsForEveryCell(world));
	});
var $author$project$World$nextGeneration = function (gameboard) {
	return _Utils_update(
		gameboard,
		{
			board: A3($author$project$World$calculateNextGeneration, gameboard.rows, gameboard.columns, gameboard.board)
		});
};
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$World$flipCellState = function (cellState) {
	if (cellState.$ === 'Alive') {
		return $author$project$World$Dead;
	} else {
		return $author$project$World$Alive;
	}
};
var $author$project$World$toggleCell = F2(
	function (index, gameboard) {
		return _Utils_update(
			gameboard,
			{
				board: A2(
					...
					F2(
						function (cellIndex, cell) {
							return _Utils_eq(index, cellIndex) ? $author$project$World$flipCellState(cell) : cell;
						}),
					gameboard.board)
			});
	});
		...
		switch (msg.$) {
			case 'ToggleCell':
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							playing: false,
							world: A2($author$project$World$toggleCell, index, model.world)
						}),
			...
			case 'NextGeneration':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							world: $author$project$World$nextGeneration(model.world)
						}),
					$elm$core$Platform$Cmd$none);
			case 'StartPlaying':
				return _Utils_Tuple2(
			...
			default:
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(model, $author$project$Model$getContentViewport);
				}
		}
	});
...
var $author$project$Messages$StartPlaying = {$: 'StartPlaying'};
var $author$project$Messages$StopPlaying = {$: 'StopPlaying'};
...
var $lattyware$elm_fontawesome$FontAwesome$Solid$pause = A5(
...
var $lattyware$elm_fontawesome$FontAwesome$Solid$play = A5(
	$lattyware$elm_fontawesome$FontAwesome$Icon$Icon,
	'fas',
	'play',
	448,
...
var $author$project$View$theme = {
	accent: {
		dark: $rtfeldman$elm_css$Css$hex('600A3B'),
		darker: $rtfeldman$elm_css$Css$hex('3E0023'),
		light: $rtfeldman$elm_css$Css$hex('9D3F74'),
		lighter: $rtfeldman$elm_css$Css$hex('C06E9C'),
		main: $rtfeldman$elm_css$Css$hex('812258')
	},
	background: $rtfeldman$elm_css$Css$hex('EEEEEE'),
	primary: {
		dark: $rtfeldman$elm_css$Css$hex('380C50'),
		darker: $rtfeldman$elm_css$Css$hex('220234'),
		light: $rtfeldman$elm_css$Css$hex('673983'),
		lighter: $rtfeldman$elm_css$Css$hex('8960A0'),
		main: $rtfeldman$elm_css$Css$hex('52206B')
	},
	secondary: {
		dark: $rtfeldman$elm_css$Css$hex('201152'),
		darker: $rtfeldman$elm_css$Css$hex('110535'),
		light: $rtfeldman$elm_css$Css$hex('503F87'),
		lighter: $rtfeldman$elm_css$Css$hex('7667A5'),
		main: $rtfeldman$elm_css$Css$hex('37266E')
	}
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Css$color($author$project$View$theme.background),
						$rtfeldman$elm_css$Css$backgroundColor($author$project$View$theme.primary.main),
						A3(
						$rtfeldman$elm_css$Css$border3,
						$rtfeldman$elm_css$Css$px(1),
						$rtfeldman$elm_css$Css$solid,
						$author$project$View$theme.primary.lighter),
						...
						_List_fromArray(
							[
								$rtfeldman$elm_css$Css$backgroundColor($author$project$View$theme.primary.light)
							]))
					])),
				$rtfeldman$elm_css$Html$Styled$Events$onClick(
				model.playing ? $author$project$Messages$StopPlaying : $author$project$Messages$StartPlaying)
			]),
		_List_fromArray(
			[
				...
				$lattyware$elm_fontawesome$FontAwesome$Icon$view(
					$lattyware$elm_fontawesome$FontAwesome$Icon$present(
						model.playing ? $lattyware$elm_fontawesome$FontAwesome$Solid$pause : $lattyware$elm_fontawesome$FontAwesome$Solid$play)))
			]));
};
var $author$project$Messages$SetSpeed = function (a) {
	return {$: 'SetSpeed', a: a};
};
...
var $author$project$View$speedSelector = function (model) {
	var toSpeedButton = function (speed) {
		return A2(
			...
			_List_fromArray(
				[
					...
					$rtfeldman$elm_css$Html$Styled$Attributes$css(
					_Utils_eq(model.speedMultiplier, speed) ? _List_fromArray(
						[
							$rtfeldman$elm_css$Css$important(
							$rtfeldman$elm_css$Css$backgroundColor($author$project$View$theme.primary.dark))
						]) : _List_Nil),
					$rtfeldman$elm_css$Html$Styled$Events$onClick(
					$author$project$Messages$SetSpeed(speed))
				]),
			_List_fromArray(
				[
					$rtfeldman$elm_css$Html$Styled$text(
					'x' + $elm$core$String$fromInt(speed))
				]));
	};
	var selectableSpeeds = _List_fromArray(
		[1, 2, 4, 8]);
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
				...
				_List_fromArray(
					[
						...
						'repeat(' + ($elm$core$String$fromInt(
							$elm$core$List$length(selectableSpeeds)) + ', 42px)')),
						A2($rtfeldman$elm_css$Css$property, 'gap', '1px'),
						$rtfeldman$elm_css$Css$height(
						$rtfeldman$elm_css$Css$px(42)),
						$rtfeldman$elm_css$Css$backgroundColor($author$project$View$theme.primary.lighter),
						$rtfeldman$elm_css$Css$color($author$project$View$theme.background),
						A3(
						$rtfeldman$elm_css$Css$border3,
						$rtfeldman$elm_css$Css$px(1),
		...
		A2($elm$core$List$map, toSpeedButton, selectableSpeeds));
};
var $rtfeldman$elm_css$Css$stretch = $rtfeldman$elm_css$Css$prop1('stretch');
var $author$project$View$actionBar = function (model) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						A2($rtfeldman$elm_css$Css$property, 'grid-template-columns', ' max-content max-content max-content'),
						A2($rtfeldman$elm_css$Css$property, 'align-items', 'center'),
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text('NATALIE\'S GAME OF LIFE - ELM')
					])),
				$author$project$View$playButton(model),
				$author$project$View$speedSelector(model)
			]));
};
var $author$project$View$colorToString = function (color) {
	return 'rgba(' + ($elm$core$String$fromInt(color.red) + (',' + ($elm$core$String$fromInt(color.green) + (',' + ($elm$core$String$fromInt(color.blue) + (',' + ($elm$core$String$fromFloat(color.alpha) + ')')))))));
};
...
var $author$project$Messages$ToggleCell = function (a) {
	return {$: 'ToggleCell', a: a};
};
var $author$project$View$world = function (model) {
	var rows = model.world.rows;
	var columns = model.world.columns;
	var _v0 = model.contentViewport;
	var viewportWidth = _v0.a;
	var viewportHeight = _v0.b;
	var squareSize = (_Utils_cmp(viewportWidth, viewportHeight) < 0) ? (((viewportWidth - rows) - 1) / rows) : (((viewportHeight - columns) - 1) / columns);
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						$rtfeldman$elm_css$Css$property,
						'grid-template-rows',
						'repeat(' + ($elm$core$String$fromInt(model.world.rows) + ', 1fr)')),
						A2(
						$rtfeldman$elm_css$Css$property,
						'grid-template-columns',
						'repeat(' + ($elm$core$String$fromInt(model.world.columns) + ', 1fr)')),
						A2($rtfeldman$elm_css$Css$property, 'gap', '1px'),
						A3(
						$rtfeldman$elm_css$Css$border3,
						$rtfeldman$elm_css$Css$px(1),
						$rtfeldman$elm_css$Css$solid,
						$author$project$View$theme.primary.main),
						$rtfeldman$elm_css$Css$backgroundColor($author$project$View$theme.primary.main),
						$rtfeldman$elm_css$Css$width(
						$rtfeldman$elm_css$Css$px(rows * squareSize)),
						$rtfeldman$elm_css$Css$height(
						$rtfeldman$elm_css$Css$px(columns * squareSize))
					]))
			]),
		A2(
			$elm$core$List$indexedMap,
			function (index) {
				return function (cellState) {
					return A2(
						$rtfeldman$elm_css$Html$Styled$div,
						_List_fromArray(
							[
								$rtfeldman$elm_css$Html$Styled$Attributes$css(
								_Utils_eq(cellState, $author$project$World$Alive) ? _List_fromArray(
									[
										$rtfeldman$elm_css$Css$backgroundColor($author$project$View$theme.secondary.main)
									]) : _List_fromArray(
									[
										$rtfeldman$elm_css$Css$backgroundColor($author$project$View$theme.background)
									])),
								$rtfeldman$elm_css$Html$Styled$Events$onClick(
								$author$project$Messages$ToggleCell(index))
							]),
						_List_Nil);
				};
			},
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Attributes$style, 'grid-template-rows', '50px 1fr '),
				A2($elm$html$Html$Attributes$style, 'grid-template-columns', '1fr'),
				A2(
				$elm$html$Html$Attributes$style,
				'background-color',
				$author$project$View$colorToString($author$project$View$theme.background))
			]),
		_List_fromArray(
			[
				A2($elm$core$Basics$composeR, $author$project$View$actionBar, $rtfeldman$elm_css$Html$Styled$toUnstyled)(model),
				$rtfeldman$elm_css$Html$Styled$toUnstyled(
				A2(
					$rtfeldman$elm_css$Html$Styled$div,
					_List_fromArray(
						[
							$rtfeldman$elm_css$Html$Styled$Attributes$id('page-content'),
							$rtfeldman$elm_css$Html$Styled$Attributes$css(
							_List_fromArray(
								[
									$rtfeldman$elm_css$Css$displayFlex,
					...
					_List_fromArray(
						[
							$author$project$View$world(model)
						])))
			]));
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{init: $author$project$Model$init, subscriptions: $author$project$Subscriptions$subscriptions, update: $author$project$Model$update, view: $author$project$View$view});
